...
const CategoriesManagementPage = () => {
  ...
  useEffect(() => {
    if (categoryToEdit) {
      setEditedCategory({
        ...
        trangThai:
          categoryToEdit.trangThai || categoryToEdit.status || "Hoạt động",
    ...
    if (statusFilter !== "Tất cả") {
      filtered = filtered.filter(
        (category) =>
          ...
          category.trangThai === statusFilter
      );
    }
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const currentPageCategories = getCurrentPageCategories().map(
        (category) => category.id || category.maDanhMuc
      );
  ...
  const handleDeleteSelected = () => {
    setCategories(
      categories.filter(
        (category) =>
          !selectedCategories.includes(category.id || category.maDanhMuc)
      )
    );
    setSelectedCategories([]);
  };
  ...
  const handleDeleteCategory = (category) => {
    if (!isAdmin) {
      Swal.fire({
        icon: "warning",
        title: "⚠️ Cảnh báo",
        text: "Chỉ quản trị viên mới được xóa danh mục.",
  ...
  const confirmDeleteCategory = async () => {
    try {
      ...
      setCategories(
        categories.filter(
          (category) => category.maDanhMuc !== categoryToDelete.maDanhMuc
        )
      );
      setIsDeleteModalOpen(false);
      setCategoryToDelete(null);
      toast.success(
        "Xóa danh mục " + categoryToDelete.tenDanhMuc + " thành công!"
      );
    } catch (error) {
  ...
  const handleEditCategory = (category) => {
    if (!isAdmin) {
      Swal.fire({
        icon: "warning",
        title: "⚠️ Cảnh báo",
        text: "Chỉ quản trị viên mới được sửa danh mục.",
        confirmButtonColor: "#d33",
        confirmButtonText: "Tôi đã hiểu",
      });
      return;
  ...
  const handleAddCategory = async (e) => {
    ...
    const errors = {};
    if (
      typeof newCategory.tenDanhMuc !== "string" ||
      !newCategory.tenDanhMuc.trim()
    ) {
    ...
    try {
      ...
      toast.error(
        "Lỗi khi thêm danh mục. Vui lòng kiểm tra lại dữ liệu hoặc thử lại sau."
      );
  ...
  const handleUpdateCategory = async (e) => {
    ...
    const errors = {};
    if (
      typeof editedCategory.tenDanhMuc !== "string" ||
      !editedCategory.tenDanhMuc.trim()
    ) {
      errors.tenDanhMuc = "Tên danh mục không được để trống";
    }
    if (Object.keys(errors).length > 0) {
    ...
    try {
      ...
      setFormErrors({});
      toast.success(
        "Cập nhật danh mục " + editedCategory.tenDanhMuc + " thành công!"
      );
    } catch (err) {
      toast.error(
        "Lỗi khi cập nhật danh mục. Vui lòng kiểm tra lại dữ liệu hoặc thử lại sau."
      );
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            <select
              ...
              className="appearance-none pl-10 pr-8 py-2 border rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full"
            >
          ...
          <button
            className="flex-shrink-0 flex items-center px-4 py-2 text-gray-700 bg-white border rounded-lg hover:bg-gray-50"
          ...
          <button
            onClick={() => {
              if (!isAdmin) {
                Swal.fire({
                  icon: "warning",
                  title: "⚠️ Cảnh báo",
                  text: "Chỉ quản trị viên mới được thêm danh mục.",
                  confirmButtonColor: "#d33",
                  confirmButtonText: "Tôi đã hiểu",
                });
                return;
            ...
            className="flex-shrink-0 flex items-center px-4 py-2 text-white bg-green-600 hover:bg-green-700 border border-green-600 rounded-lg"
          >
            ...
            <input
              ...
              checked={
                selectedCategories.length ===
                  getCurrentPageCategories().length &&
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {getCurrentPageCategories().map((category) => (
                <tr
                  key={category.maDanhMuc || category.id}
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input
                      ...
                      onChange={() =>
                        handleSelectCategory(category.maDanhMuc || category.id)
                      }
                    />
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div
                        ...
                        onClick={() =>
                          handleViewImage(
                            category.hinhAnh || "/placeholder.svg"
                          )
                        }
                      >
                        <img
                      ...
                      <div className="ml-4">
                        ...
                        <div className="text-sm text-gray-500">
                          {category.moTa || category.description}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      ...
                      ${
                        (category.trangThai || category.status) === "Hoạt động"
                      ...
                      {editedCategory.hinhAnh ? (
                        <div className="mb-3 relative group">
                          <img
                            ...
                            className="h-32 w-32 object-cover rounded-lg border border-gray-300"
                            onClick={() =>
                              handleViewImage(editedCategory.hinhAnh)
                            }
                          />
                          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 flex items-center justify-center transition-all rounded-lg">
                            <span className="text-white opacity-0 group-hover:opacity-100 font-medium">
            ...
            <div className="relative bg-white rounded-lg shadow-lg max-w-md w-full p-6 z-20">
              ...
              <div className="mb-6">
                <p className="text-gray-700">
                  ...
                  <span className="font-medium">
                    {categoryToDelete?.tenDanhMuc || categoryToDelete?.name}
                  </span>
