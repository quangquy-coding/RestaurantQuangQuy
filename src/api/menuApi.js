...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL; // Thay đổi theo URL API của bạn
...
export const searchDishes = async (tenMon, maDanhMuc, tinhTrang) => {
  try {
    ...
    const response = await api.get(
      `/MonAnManager/TimKiem?${params.toString()}`
    );
    return response.data;
  } catch (error) {
...
export const filterByCategory = async (maDanhMuc) => {
  try {
    const response = await api.get(
      `/MonAnManager/LocTheoDanhMuc?maDanhMuc=${maDanhMuc}`
    );
    return response.data;
  } catch (error) {
